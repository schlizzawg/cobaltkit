...
check_ip() {
    ...
    PUBLIC_IP=$(curl -${VERSION}s https://icanhazip.com)

    [ "$PUBLIC_IP" = "" ] && {
        echo "[?] server has no ipv$VERSION, skipping check"
        return
    }
    ...
    [ "$COBALT_MODE" = "both" ] && {
        ...
        [ "$WEB_IP" != "$PUBLIC_IP" ] && {
            echo "[!] $DNS_TYPE record '$WEB_IP' for $WEB_DOMAIN does not match public-facing IPv$VERSION '$PUBLIC_IP'" >&2
            IP_ERR=1
        }
    }
    ...
    [ "$API_IP" != "$PUBLIC_IP" ] && {
        echo "[!] $DNS_TYPE record '$API_IP' for $API_DOMAIN does not match public-facing IPv$VERSION '$PUBLIC_IP'" >&2
        IP_ERR=1
    }

    [ "$IP_ERR" = 1 ] && {
        echo "    If you want your instance to support IPv$VERSION, you may want to fix this; otherwise, you may ignore it"
...
check_dns() {
    command -v curl >/dev/null 2>/dev/null || { echo 'no curl available, skipping dns check'; return; }
